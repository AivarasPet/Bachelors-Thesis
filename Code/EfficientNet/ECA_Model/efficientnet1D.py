...
def block(
    ...
    # ECA phase
    if 0 < k_size:  # k_size is the kernel size for ECA, you can set it to 3 or 5
        eca = layers.GlobalAveragePooling2D(name=name + "eca_squeeze")(x)
        eca = layers.Reshape((filters, 1), name=name + "eca_reshape")(eca)
        eca = layers.Conv1D(
            1,
            k_size,
            padding="same",
        ...
        eca = layers.Reshape((1, 1, filters), name=name + "eca_expand")(eca)
        x = layers.multiply([x, eca], name=name + "eca_excite")
